...
@section('title', 'Edit Indikator Kinerja - Admin')
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Indikator Kinerja</h2>
            <p class="text-sm text-gray-600 mt-1">Edit data indikator kinerja</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.indikator-kinerja.update', $indikator->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="sasaran_kegiatan_id" id="sasaran_kegiatan_id" required
                    ...
                    @foreach($sasaran as $item)
                        <option value="{{ $item->id }}" {{ old('sasaran_kegiatan_id', $indikator->sasaran_kegiatan_id) == $item->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="unit_kerja_id" id="unit_kerja_id"
                    ...
                    @foreach($unitKerja as $unit)
                        <option value="{{ $unit->id }}" {{ old('unit_kerja_id', $indikator->unit_kerja_id) == $unit->id ? 'selected' : '' }}>
                            {{ $unit->nama_unit }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="kode_indikator" id="kode_indikator" value="{{ old('kode_indikator', $indikator->kode_indikator) }}" required
            ...
            <div>
                ...
                <textarea name="nama_indikator" id="nama_indikator" rows="3" required
                    ...
                    placeholder="Nama indikator kinerja / RHK yang akan dipilih ASN">{{ old('nama_indikator', $indikator->nama_indikator) }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="satuan" id="satuan" value="{{ old('satuan', $indikator->satuan) }}" required
            ...
            <div>
                ...
                <select name="tipe_target" id="tipe_target" required
                    ...
                    <option value="ANGKA" {{ old('tipe_target', $indikator->tipe_target) === 'ANGKA' ? 'selected' : '' }}>ANGKA</option>
                    <option value="DOKUMEN" {{ old('tipe_target', $indikator->tipe_target) === 'DOKUMEN' ? 'selected' : '' }}>DOKUMEN</option>
                    <option value="PERSENTASE" {{ old('tipe_target', $indikator->tipe_target) === 'PERSENTASE' ? 'selected' : '' }}>PERSENTASE</option>
                </select>
                @error('tipe_target')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
