...
@section('title', 'Tambah Sasaran Kegiatan - Admin')
@section('page-title', 'Tambah Sasaran Kegiatan')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Tambah Sasaran Kegiatan</h2>
            <p class="text-sm text-gray-600 mt-1">Tambah sasaran kegiatan baru</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.sasaran-kegiatan.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_sasaran" id="nama_sasaran" value="{{ old('nama_sasaran') }}" required
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                    ...
                    placeholder="Deskripsi sasaran kegiatan">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
