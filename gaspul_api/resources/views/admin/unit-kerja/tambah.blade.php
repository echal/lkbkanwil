        ...
        <form action="{{ route('admin.unit-kerja.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="parent_id" id="parent_id"
                    ...
                    @foreach($parentOptions as $opt)
                        <option value="{{ $opt['id'] }}"
                            {{ old('parent_id', $defaultParentId ?? '') == $opt['id'] ? 'selected' : '' }}>
                            {{ $opt['label'] }}
                        </option>
                    @endforeach
                </select>
                @error('parent_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-400">Level unit akan dihitung otomatis dari unit induk yang dipilih.</p>
            </div>
            ...
            <div>
                ...
                <select name="hari_kerja" id="hari_kerja" required
                    ...
                    <option value="SENIN_JUMAT" {{ old('hari_kerja', 'SENIN_JUMAT') === 'SENIN_JUMAT' ? 'selected' : '' }}>5 Hari (Senin–Jumat)</option>
                    <option value="SENIN_SABTU" {{ old('hari_kerja') === 'SENIN_SABTU' ? 'selected' : '' }}>6 Hari (Senin–Sabtu)</option>
                </select>
