...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Edit data pegawai</p>
        </div>
        ...
        <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="unit_kerja_id" id="unit_kerja_id"
                    ...
                    @foreach($units as $opt)
                        <option value="{{ $opt['id'] }}" {{ old('unit_kerja_id', $pegawai->unit_kerja_id) == $opt['id'] ? 'selected' : '' }}>
                            {{ $opt['label'] }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="role" id="role" required
                    ...
                    <option value="ASN" {{ old('role', $pegawai->role) === 'ASN' ? 'selected' : '' }}>ASN</option>
                    <option value="ATASAN" {{ old('role', $pegawai->role) === 'ATASAN' ? 'selected' : '' }}>ATASAN</option>
