...
<head>
    ...
    <title>Laporan Kinerja Harian (LKH) - {{ $asn->name }}</title>
    <style>
        @media print {
            @page {
                size: A4;
...
<body>
    ...
    <!-- Header -->
    <div class="header">
        <h1>Laporan Kinerja Harian (LKH)</h1>
    ...
    <p style="text-align: center; padding: 40px 0; color: #666;">
        Tidak ada data Kinerja Harian untuk tanggal ini.
    </p>
    @else
    <table class="content-table">
        ...
        <tbody>
            @php
                $totalMenit = 0;
            @endphp
            @foreach($data_lkh as $index => $lkh)
            @php
                $totalMenit += $lkh->durasi_menit;
            @endphp
            <tr>
                <td class="center">{{ $index + 1 }}</td>
                <td class="center">
                    {{ \Carbon\Carbon::parse($lkh->jam_mulai)->format('H:i') }} -
                    {{ \Carbon\Carbon::parse($lkh->jam_selesai)->format('H:i') }}<br>
                    <small>({{ floor($lkh->durasi_menit / 60) }}j {{ $lkh->durasi_menit % 60 }}m)</small>
                </td>
                <td>{{ $lkh->rencana_kegiatan_harian }}</td>
                <td class="center">{{ $lkh->progres }} {{ $lkh->satuan }}</td>
                <td class="center">
                    @if($lkh->status_bukti === 'SUDAH_ADA')
                        <small>{{ $lkh->bukti_dukung }}</small>
                    @else
