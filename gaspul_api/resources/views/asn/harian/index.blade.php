...
<div class="space-y-6" x-data="{
    ...
    <!-- Keterangan Warna & Status Kalender -->
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <h3 class="text-sm font-semibold text-gray-700 mb-4">Keterangan Status Kalender:</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded bg-blue-600 flex items-center justify-center">
                    <span class="text-white text-xs font-bold">LKH</span>
                </div>
                <span class="text-sm text-gray-700">LKH Terisi</span>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded bg-purple-600 flex items-center justify-center">
                    <span class="text-white text-xs font-bold">RHK</span>
                </div>
                <span class="text-sm text-gray-700">RHK Ada</span>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded bg-green-50 border-2 border-green-200 flex items-center justify-center">
                    <span class="text-green-700 text-xs font-bold">✓</span>
                </div>
                <span class="text-sm text-gray-700">Hari Kerja</span>
            </div>
            <div class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded bg-red-100 border-2 border-red-300 flex items-center justify-center">
                    <span class="text-red-700 text-xs font-bold">L</span>
                </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm text-gray-700">Tidak Bisa Input</span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <!-- Calendar Grid (Menggunakan Partial Baru) -->
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div class="flex items-start justify-between mb-2">
                                ...
                                <div class="flex items-center space-x-1">
                                    @if($entry['tipe_progres'] === 'TUGAS_ATASAN')
                                    {{-- TLA: Route tanpa middleware skp.approved --}}
                                    <a href="{{ route('asn.harian.edit-tla', $entry['id']) }}?date={{ $selectedDate }}"
                                       class="p-1 hover:bg-gray-200 rounded transition text-blue-600" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <button @click="deleteEntry = { id: {{ $entry['id'] }}, date: '{{ $selectedDate }}', type: 'tla' }"
                                    ...
                                    @else
                                    {{-- Kinerja Harian: Route dengan middleware skp.approved --}}
                                    ...
                                    <button @click="deleteEntry = { id: {{ $entry['id'] }}, date: '{{ $selectedDate }}', type: 'kh' }"
                                        class="p-1 hover:bg-gray-200 rounded transition text-red-600" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
    ...
    <div x-show="deleteEntry !== null"
        ...
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            ...
            <div class="flex items-center justify-end space-x-3 mt-6">
                ...
                <form x-bind:action="deleteEntry ? (deleteEntry.type === 'tla' ? '{{ url('/asn/harian/destroy-tla') }}/' : '{{ url('/asn/harian/destroy') }}/') + deleteEntry.id + '?date=' + deleteEntry.date : ''"
...
@push('scripts')
<script>
// Validasi Weekend & Hari Libur untuk Form Input
document.addEventListener('DOMContentLoaded', function() {
    ...
    function canInputDate(dateString) {
        ...
        if (holidays[dateString]) {
            return {
                canInput: false,
                reason: `Hari Libur: ${holidays[dateString]}`
            };
        }

        // Check tanggal masa depan
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        if (date > today) {
            return {
                canInput: false,
                reason: 'Tanggal masa depan - Belum dapat input'
            };
        }

        return { canInput: true, reason: null };
    }

    // Monitor perubahan tanggal di form
    const dateInputs = document.querySelectorAll('input[type="date"][name="tanggal"]');

    dateInputs.forEach(input => {
        // Disable weekend & holiday saat load
        validateDateInput(input);

        // Validate saat berubah
    ...
    function validateDateInput(inputElement) {
        ...
        const validation = canInputDate(selectedDate);
        const form = inputElement.closest('form');
        const submitButton = form ? form.querySelector('button[type="submit"]') : null;
        const warningDiv = form ? form.querySelector('.date-warning') : null;

        if (!validation.canInput) {
            // Disable submit button
            if (submitButton) {
                submitButton.disabled = true;
                submitButton.classList.add('opacity-50', 'cursor-not-allowed');
                submitButton.title = validation.reason;
            }
            ...
            if (warningDiv) {
                ...
                newWarning.className = 'date-warning bg-yellow-50 border border-yellow-300 text-yellow-800 px-4 py-2 rounded text-sm mt-2';
                newWarning.textContent = validation.reason;
                inputElement.parentElement.appendChild(newWarning);
            }
        } else {
    ...
    // Set min/max date untuk input tanggal
    dateInputs.forEach(input => {
        // Max date = hari ini
        const today = new Date().toISOString().split('T')[0];
        input.setAttribute('max', today);

        // Min date = 3 bulan yang lalu (opsional)
