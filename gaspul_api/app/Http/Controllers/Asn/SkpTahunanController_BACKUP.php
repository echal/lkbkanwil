...
/**
 * Controller SKP Tahunan ASN - EMERGENCY FALLBACK (No Policy)
 */
class SkpTahunanController extends Controller
{
     ...
     * EMERGENCY VERSION - No Policy (Direct Check)
     */
    public function edit($id)
    {
        $asn = auth()->user();

        // Load detail dengan explicit ownership check
        $detail = SkpTahunanDetail::with(['skpTahunan', 'indikatorKinerja.sasaranKegiatan'])
            ->findOrFail($id);

        // MANUAL SECURITY CHECK (No Policy)
