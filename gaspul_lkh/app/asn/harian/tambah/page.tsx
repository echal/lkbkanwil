...
import { createHarian } from '@/app/lib/harian-api';
import { getAvailableBulanan, Bulanan } from '@/app/lib/bulanan-api';

export default function TambahHarianPage() {
  ...
  const [selectedBulanan, setSelectedBulanan] = useState<Bulanan | null>(null);
  ...
  const loadBulananList = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getAvailableBulanan();
      setBulananList(data);

      if (data.length === 0) {
        setError(
          'Tidak ada Bulanan yang tersedia. Anda harus mengisi target Bulanan terlebih dahulu.'
        );
      }
    } catch (err: any) {
      setError(err.message || 'Gagal memuat Bulanan');
  ...
  useEffect(() => {
    if (formData.bulanan_id > 0) {
      const bulanan = bulananList.find((b) => b.id === formData.bulanan_id);
      setSelectedBulanan(bulanan || null);
      if (bulanan) {
        // Set default satuan from SKP
        setFormData((prev) => ({
          ...prev,
          satuan: bulanan.rencana_kerja_asn?.satuan || '',
        }));
      }
    } else {
      setSelectedBulanan(null);
    }
  }, [formData.bulanan_id, bulananList]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (formData.bulanan_id === 0) {
      alert('Pilih Bulanan terlebih dahulu');
      return;
    }
    ...
    if (formData.bukti_type === 'file' && !formData.bukti_file) {
    ...
    try {
      setSaving(true);
      await createHarian(formData);
      alert('Kegiatan Harian berhasil ditambahkan!');
      router.push('/asn/harian');
    } catch (err: any) {
      alert(err.message || 'Gagal menambahkan Harian');
    } finally {
      setSaving(false);
    }
  };
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    >
                      Pilih Bulanan <span className="text-red-500">*</span>
                    </label>
                    <select
                      id="bulanan_id"
                      value={formData.bulanan_id}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          bulanan_id: parseInt(e.target.value),
                        })
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-900"
                      required
                    >
                      <option value="0">-- Pilih Bulanan --</option>
                    ...
                    {selectedBulanan && (
                      <p className="mt-1 text-xs text-gray-500">
                        Target: {selectedBulanan.target_bulanan}{' '}
                        {selectedBulanan.rencana_kerja_asn?.satuan} | Realisasi:{' '}
                        {selectedBulanan.realisasi_bulanan}{' '}
                        {selectedBulanan.rencana_kerja_asn?.satuan} ({selectedBulanan.capaian_persen}
                        %)
                      </p>
                    )}
                  </div>
                  ...
                  {formData.bukti_type === 'link' ? (
                    ...
                    <div>
                      ...
                      <input
                        ...
                        required={formData.bukti_type === 'file'}
                      />
                      <p className="mt-1 text-xs text-gray-500">
                      ...
                      {formData.bukti_file && (
                        <p className="mt-2 text-sm text-green-600">
                          File terpilih: {formData.bukti_file.name} (
